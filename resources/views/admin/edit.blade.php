...
<form action="/admin/update/{{ $movie->id }}" method="POST">
    @csrf
    <div>
        映画名：<input type="text" id="name" name="name" value="{{ old('name',$movie->name) }}">
    ...
    <div>
        映画情報：<input type="text" id="name" name="information" value="{{ old('information',$movie->information) }}">
        {{-- name フィールドにエラーがある場合、エラーメッセージを表示 --}}
    ...
    <div>
        カテゴリID：<input type="number" name="category_id" value="{{ old('category_id',$movie->category_id) }}" min="1" max="5"
            step="1">
    </div>
    <div>
        シリーズID：<input type="number" name="series_id" value="{{ old('series_id',$movie->series_id) }}" min="1" max="5"
            step="1">
    </div>
    <div>
        視聴回数：<input type="number" name="view" value="{{ old('view',$movie->view) }}" min="0" max="10000000000"
            step="10000">
    </div>
    <div>
        <label>
